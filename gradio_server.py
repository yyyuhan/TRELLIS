...
# For this example, a mock function to simulate 3D asset generation.
def generate_assets_from_image(image, use_process_pool=False, generate_mesh=False):
    if image is None:
...
def generate_assets_from_outputs(outputs):
    mp4_files = []
    files_to_download = []
    video_gaussian = render_utils.render_video(outputs['gaussian'][0])['color']
    ply_data = outputs['gaussian'][0]
    with tempfile.NamedTemporaryFile(suffix='.mp4', delete=False) as temp_mp4_gaussian, \
        ...
        imageio.mimsave(temp_mp4_gaussian.name, video_gaussian, fps=30)
        ply_data.save_ply(temp_ply.name)
        mp4_files.append(temp_mp4_gaussian.name)
        files_to_download.append(temp_ply.name)
        schedule_delete(temp_mp4_gaussian.name)
        schedule_delete(temp_ply.name)
    return files_to_download, mp4_files
